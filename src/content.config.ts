...
const education = defineCollection({
  loader: glob({ pattern: "**/*.{md,mdoc,yaml}", base: "./src/content/education" }),
  schema: ({ image }) =>
    z.object({
      title: z.string(),
      subtitle: z.string(),
...
const projectCategories = defineCollection({
  loader: glob({ pattern: "**/*.{md,mdoc,yaml}", base: "./src/content/projectCategories" }),
  schema: z.object({
    title: z.string(),
...
const hackathons = defineCollection({
  loader: glob({ pattern: "**/*.{md,mdoc,yaml}", base: "./src/content/hackathons" }),
  schema: ({ image }) =>
    z.object({
      title: z.string(),
      location: z.string(),
...
// Blog collection —— image 已设为可选，无需封面图也能发布
  ...
  schema: z.object({
    ...
    extraLinks: z.array(
      z.object({
        link: z.string(),
        icon: z.enum([
          "Flower2", "BookOpen", "FileText", "CodeXml", "Mail",
          "Home", "User", "Briefcase", "GraduationCap", "Link",
        ]),
        label: z.string(),
    ...
    projectsLayout: z.enum(["grid", "tabs-horizontal", "tabs-vertical"]).optional().default("grid"),
  }),
});
